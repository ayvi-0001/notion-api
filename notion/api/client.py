...
class _NotionClient:
    ...
    @staticmethod
    def _block_endpoint(object_id: Optional[str] = None, 
                        /, *, 
    ...
    def _get(self, url: NotionEndpoint, /, *, 
             payload: Optional[Union[JSONObject, JSONPayload]] = None) -> JSONObject:
        if payload is None:
    ...
    def _patch(self, url: NotionEndpoint, /, *, 
               payload: Union[JSONObject, JSONPayload]) -> JSONObject:
        if isinstance(payload, dict):
            payload = orjson.dumps(payload)
        response = orjson.loads(
